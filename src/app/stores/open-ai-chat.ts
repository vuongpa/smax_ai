...
import { BehaviorSubject, Observable, distinctUntilChanged, pipe, switchMap, tap } from "rxjs";
...
export class OpenAiChatStore extends ComponentStore<OpenAiChatState> {
  ...
  readonly activeConversation$: Observable<Conversation | undefined> = this.select(state => state.conversations.find(c => c.active));
  ...
  readonly sendMessageEffect = this.effect<string>(pipe(
    tap((message: string) => {
      this.addMessage(message, SENDER.HUMAN);
      this.scrollSubject.next(uuid.v4());
    }),
    switchMap((message: string) => this.openAiService.ask(message).pipe(
        tapResponse(
          (response) => {
